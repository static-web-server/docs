...
**Be aware of the security implications** when enabling this option. Read the [Trade-offs of enabling `--follow-symlinks`](#trade-offs-of-enabling-follow-symlinks) section below for more details.
...
When `--follow-symlinks` is disabled (default), the symlink check calls [`symlink_metadata`][symlink_metadata] (equivalent to `lstat(2)`) once per path component to determine whether each segment is a symlink. For a request like `/assets/images/photo.jpg`, that's three lightweight metadata syscalls.
...
- Operating-system inode caches make `lstat` extremely fast on repeat access.
- The containment cache absorbs the dominant cost (`canonicalize`) on the hot path.

## Trade-offs of enabling `--follow-symlinks` {#trade-offs-of-enabling-follow-symlinks}
